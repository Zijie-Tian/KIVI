...
kv_ref_filename = os.path.join(project_dir, f"{MODEL_NAME}_kvcache_ref.pt")

kvcache = torch.load(kv_filename, map_location="cpu", weights_only=False)
kvcache_ref = torch.load(kv_ref_filename, map_location="cpu", weights_only=False)
...
for layer_id in [3, 8, 14, 16, 18, 20, 31]:  # Replace with your layer ids
    ...
    k_ref, v_ref = (
        kvcache_ref[layer_id][0].squeeze(0),
        kvcache_ref[layer_id][1].squeeze(0),
    )
    ...
    k_ref = k_ref.transpose(0, 1).abs().detach().numpy()
    v_ref = v_ref.transpose(0, 1).abs().detach().numpy()
    k, v = k[:, head_id, :], v[:, head_id, :]
    k_ref, v_ref = k_ref[:, head_id, :], v_ref[:, head_id, :]

    # Iterate over key and value tensors for kvcache
    ...
    # Iterate over key and value tensors for kvcache_ref
    for idx, tensor in enumerate([k_ref, v_ref]):
        tokens, channels = tensor.shape
        x = np.arange(tokens)
        y = np.arange(channels)
        X, Y = np.meshgrid(x, y)
        ...
        if idx == 0:
            filename = f"{MODEL_NAME}_layer{layer_id}_head{head_id}_k_ref.pdf"
        else:
            filename = f"{MODEL_NAME}_layer{layer_id}_head{head_id}_v_ref.pdf"
