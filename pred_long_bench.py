...
if __name__ == "__main__":
    ...
    # model_max_length=training_args.model_max_length)
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_args.model_name_or_path, use_fast=True, trust_remote_code=True
        )
    ...
    if (
        ...
        if model_args.k_bits < 16 and model_args.v_bits < 16:
            ...
            config.use_flash = (
                True  # Note: We activate the flashattention to speed up the inference
            )
    ...
    for dataset in datasets:
        if data_args.e:
            data = load_dataset(
                "/workspace/data/LongBench", f"{dataset}_e", split="test"
            )
            if not os.path.exists(
                f"pred_e/{model_name}_{max_length}_{model_args.k_bits}bits_group{model_args.group_size}_residual{model_args.residual_length}"
            ):
                os.makedirs(
                    f"pred_e/{model_name}_{max_length}_{model_args.k_bits}bits_group{model_args.group_size}_residual{model_args.residual_length}"
                )
            ...
            data = load_dataset("/workspace/data/LongBench", dataset, split="test")
            if not os.path.exists(
                f"pred/{model_name}_{max_length}_{model_args.k_bits}bits_group{model_args.group_size}_residual{model_args.residual_length}"
            ):
                os.makedirs(
                    f"pred/{model_name}_{max_length}_{model_args.k_bits}bits_group{model_args.group_size}_residual{model_args.residual_length}"
                )
